...
export const getNotionFromDB = async (userId: string) => {
  const db = await getDB();
  return db.query.notionConnectionsTable.findFirst({
...
/**
 * This function can only be called in a Server @/components/commerce, Server Action or Route Handler
 */
