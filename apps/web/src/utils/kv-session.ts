...
import { getNotionFromDB, getUserFromDB } from "@/utils/auth";
...
type KVSessionNotion = Exclude<Awaited<ReturnType<typeof getNotionFromDB>>, undefined>;
...
export interface KVSession {
  ...
  createdAt: number;
  user: KVSessionUser;
  notion?: KVSessionNotion;
  country?: string;
  city?: string;
  continent?: string;
  ip?: string | null;
  userAgent?: string | null;
  authenticationType?: "passkey" | "password" | "google-oauth" | "notion-oauth";
  passkeyCredentialId?: string;
}

export async function getKV() {
  // if (process.env.NEXT_CACHE_WORKERS_KV) {
  //   return process.env.NEXT_CACHE_WORKERS_KV;
  // }
